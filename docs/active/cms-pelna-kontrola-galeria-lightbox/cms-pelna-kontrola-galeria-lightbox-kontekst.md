...
- **Unit 4 ✅** (2026-05-30): pełny gate. `test:int` 13/13, typecheck 0 błędów, `lint` 0 błędów (5 pre-existing warningów, brak nowych), `generate:types` OK, `pnpm build` exit 0 (24 strony, `/galeria` SSG). Grep `@/data/gallery|imageUrl|.ratio` czysty — brak martwych referencji. **Wszystkie 4 Unity ukończone.** Pozostają tylko scenariusze E2E do wizualnej weryfikacji w `/dev-docs-review`.
